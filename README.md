...
- **路由出站选择** — 启用 `trojan_route` 后按路由顺序匹配目标域名/IP/端口，每条路由可配置多个直连或 SOCKS5 出站并随机选择一个
...
| 语法 | 默认 | 上下文 |
|:---|:---|:---|
| `trojan_geoip <path>;` | — | `server` |

加载用户上传到服务器的 `geoip.dat` 文件，用于解析 `trojan_routes` 里的 `geoip:<code>` 规则。文件格式为 v2fly/geoip 生成的 protobuf `GeoIPList`；模块在加载 Nginx 配置时解析该文件，运行时不读取磁盘。
...
| 语法 | 默认 | 上下文 |
|:---|:---|:---|
| `trojan_route on \| off;` | `off` | `server` |
| `trojan_routes { ... }` | — | `server` |

`trojan_route on;` 启用真正的路由选择。启用后必须至少配置一个 `trojan_routes` 路由块；处理请求时按路由块出现顺序匹配目标，命中后从该路由块内的出站列表随机选择一个出站。若没有任何路由匹配，请求会被直接关闭，不再回退到直连。
...
|:---|:---|
| `rule <expr>;` | 添加一条路由规则 |
| `rules <expr> [...];` | 一次添加多条路由规则 |
| `outbounds_direct [ip_prefer=<auto\|ipv4\|ipv6>] [block=<none\|h3\|udp\|all>];` | 添加一个直连出站 |
| `outbounds_socks5 <server> [username=<string>] [password=<string>] [block=<none\|h3\|udp>];` | 添加一个 SOCKS5 出站，`server` 默认端口 1080 |
| `outbounds socks5 <server> ...;` | `outbounds_socks5` 的别名 |

路由出站选项也可写成空格分隔的 `key value` 形式，例如 `outbounds_direct ip_prefer ipv4;`。

路由规则按“任意一条命中即匹配该路由”处理，支持：

| 规则 | 说明 |
|:---|:---|
| `*` | 匹配任意目标 |
| `domain:example.com` | 大小写不敏感匹配域名本身或其子域名后缀 |
| `geosite:netflix` | 使用 `trojan_geosite` 加载的 geosite 分组匹配域名；支持 `geosite:name@attr` / `geosite:name@!attr` |
| `geoip:cn` | 使用 `trojan_geoip` 加载的 geoip 分组匹配字面量 IPv4/IPv6 目标 |
| `ip:1.2.3.4` / `ip:1.2.3.0/24` | 匹配字面量 IPv4/IPv6 地址或 CIDR |
| `port:443` / `port:1000-2000` | 匹配目标端口或端口范围 |

未启用 `trojan_route` 时，server 级 `outbounds_direct` / `outbounds_socks5` 仍按旧兼容路径工作：取第一个可用出站；若未配置 server 级出站则直连。
...
stream {
    server {
        ...
        # 路由 1：Netflix / Google / CN IP / 指定 IP 段 / 指定端口
        trojan_routes {
            rules geosite:netflix domain:google.com geoip:cn
                  ip:1.2.3.0/24 port:1000-2000;

            outbounds_socks5 aaa.com:12345 username=asd password=asd;
            outbounds_socks5 bbb.com:12345;
        }

        # 兜底路由：未命中前面规则的目标直连；删除该路由则未匹配请求会被关闭
        trojan_routes {
            rule *;
...
- `geosite` 规则依赖 `trojan_geosite` 指向的真实 `geosite.dat`；`geoip` 路由规则依赖 `trojan_geoip` 指向的真实 `geoip.dat`。文件只在加载/重载 Nginx 配置时读取，替换文件后需 reload。
- `geoip` 和 `ip` 路由只匹配客户端请求中的字面量 IPv4/IPv6 目标；域名目标不会先解析成 IP 再回到路由阶段。
...
| `ngx_stream_trojan_geoip.c/h` | geoip.dat 解析与真实 GeoIP CIDR 匹配 |
