...
- **Mux 多路复用** — 支持 Trojan-Go `0x7f` mux 与 Xray/V2Ray `v1.mux.cool` Mux.Cool，将多个 TCP CONNECT 流复用在同一个 Trojan 连接内
...
| `ngx_stream_trojan_mux.c/h` | Mux 帧编解码：smux v1、Mux.Cool 元数据、流请求长度判定 |
