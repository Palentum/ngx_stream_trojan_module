...
- **DNS 解析链路**：直连域名目标按 `trojan_dns_rules` → `trojan_doh` → NGINX `resolver` → 系统解析的顺序处理；DoH 查询失败不会静默回退到下一层。
...
5. 直连域名解析按 DNS 规则、DoH、NGINX resolver、系统解析推进。
...
| `trojan_dns_rules <path>;` | — | 加载 DNS 规则文件，按目标域名选择 resolver 和 IPv4/IPv6 策略。 |
...
- DNS 服务器组行以 `:` 结尾，多个服务器用逗号分隔，交给 NGINX resolver 创建。
- 每个组必须至少包含一条规则。
...
- `trojan_doh` 查询失败会结束本次请求，不回退到 NGINX resolver 或系统解析。
