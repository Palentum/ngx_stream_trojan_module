...
#define NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE 10
...
#ifndef NGX_STREAM_TROJAN_ROUTE_CACHE_ENTRIES
#define NGX_STREAM_TROJAN_ROUTE_CACHE_ENTRIES 1024
#endif
#define NGX_STREAM_TROJAN_ROUTE_CACHE_WAYS 4
#if (NGX_STREAM_TROJAN_ROUTE_CACHE_ENTRIES < NGX_STREAM_TROJAN_ROUTE_CACHE_WAYS) \
    || (NGX_STREAM_TROJAN_ROUTE_CACHE_ENTRIES % NGX_STREAM_TROJAN_ROUTE_CACHE_WAYS)
#error "NGX_STREAM_TROJAN_ROUTE_CACHE_ENTRIES must be >= WAYS and divisible by WAYS"
#endif
#define NGX_STREAM_TROJAN_ROUTE_CACHE_SETS \
    (NGX_STREAM_TROJAN_ROUTE_CACHE_ENTRIES \
     / NGX_STREAM_TROJAN_ROUTE_CACHE_WAYS)
#ifndef NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_ENTRIES
#define NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_ENTRIES 1024
#endif
#define NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_WAYS 4
#if (NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_ENTRIES < NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_WAYS) \
    || (NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_ENTRIES % NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_WAYS)
#error "NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_ENTRIES must be >= WAYS and divisible by WAYS"
#endif
#define NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_SETS \
    (NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_ENTRIES \
     / NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_WAYS)
#define NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_MAX_TTL 30000
...
typedef struct {
    ...
    ngx_uint_t                  hash;
    ngx_str_t                   value;
    ngx_uint_t                  route_index;
    ngx_stream_trojan_route_t  *route;
} ngx_stream_trojan_route_domain_index_t;
...
typedef struct {
    ...
    uintptr_t                   resolver_id;
    struct sockaddr_storage     sockaddr;
    socklen_t                   socklen;
} ngx_stream_trojan_udp_domain_cache_entry_t;
...
struct ngx_stream_trojan_srv_conf_s {
    ...
    ngx_stream_trojan_route_domain_index_t *route_domain_index;
    ngx_uint_t   route_domain_index_slots;
    ngx_stream_trojan_udp_domain_cache_entry_t *udp_domain_cache;
    ngx_uint_t   udp_domain_cache_generation;
...
typedef struct {
    ...
    ngx_uint_t                      ip_prefer;
    uintptr_t                       udp_cache_key;
    ngx_msec_t                      udp_cache_expire;
    ngx_stream_trojan_doh_conf_t   *doh_conf;
    u_char                         *name;
    size_t                          name_len;
    uint16_t                        qtype;
...
typedef struct ngx_stream_trojan_mux_pending_frame_s
    ngx_stream_trojan_mux_pending_frame_t;

struct ngx_stream_trojan_mux_pending_frame_s {
    ngx_queue_t                         queue;
    size_t                              pos;
    size_t                              len;
    size_t                              capacity;
...
struct ngx_stream_trojan_mux_stream_s {
    ...
    ngx_queue_t                         update_queue;
    ngx_queue_t                         pending_frames;
    ngx_stream_trojan_ctx_t            *ctx;
    ngx_pool_t                         *pool;
    uint32_t                            id;
    ...
    ngx_uint_t                          update_queued;
    ngx_uint_t                          update_resend;
    u_char                              update_frame[
        NGX_STREAM_TROJAN_MUX_HEADER_LEN + NGX_STREAM_TROJAN_MUX_UPD_LEN];
    size_t                              update_frame_pos;
    size_t                              update_frame_len;
    uint32_t                            rx_consumed;
    uint32_t                            rx_since_update;
    uint32_t                            tx_written;
    uint32_t                            tx_peer_consumed;
    uint32_t                            tx_peer_window;
    uint32_t                            update_consumed;
    size_t                              frame_payload_len;
    size_t                              frame_payload_sent;

    u_char                              frame_header[
        NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE
...
struct ngx_stream_trojan_ctx_s {
    ...
    ngx_uint_t                  mux_payload_deferred;
    uint8_t                     mux_version;
    size_t                      mux_recv_tokens;
...
static ngx_int_t ngx_stream_trojan_start_resolver(ngx_stream_trojan_ctx_t *ctx,
    ...
    ngx_uint_t ip_prefer, uintptr_t udp_cache_key);
...
static ngx_int_t ngx_stream_trojan_route_match(
    ...
    ngx_uint_t skip_domain);
static ngx_int_t ngx_stream_trojan_route_rule_match(
    ngx_stream_trojan_route_rule_t *rule, ngx_stream_trojan_addr_t *target);
static ngx_uint_t ngx_stream_trojan_route_domain_hash(const u_char *value,
    size_t value_len);
static ngx_uint_t ngx_stream_trojan_route_domain_index_slots(
    ngx_uint_t count);
static ngx_int_t ngx_stream_trojan_route_build_domain_index(ngx_conf_t *cf,
    ngx_stream_trojan_srv_conf_t *conf);
static ngx_int_t ngx_stream_trojan_route_domain_index_insert(
    ngx_stream_trojan_route_domain_index_t *index, ngx_uint_t slots,
    ngx_str_t *value, ngx_uint_t route_index,
    ngx_stream_trojan_route_t *route);
static ngx_stream_trojan_route_domain_index_t *
ngx_stream_trojan_route_domain_index_find(ngx_stream_trojan_srv_conf_t *conf,
    const u_char *value, size_t value_len, ngx_uint_t hash);
static ngx_stream_trojan_route_t *ngx_stream_trojan_route_domain_index_match(
    ngx_stream_trojan_srv_conf_t *conf, ngx_stream_trojan_addr_t *target,
    ngx_uint_t *route_index);
...
static ngx_int_t ngx_stream_trojan_mux_stream_can_accept(
    ngx_stream_trojan_mux_stream_t *stream, size_t len);
static ngx_int_t ngx_stream_trojan_mux_prepare_client_payload(
    ngx_stream_trojan_ctx_t *ctx, ngx_stream_trojan_mux_stream_t *stream,
    size_t len, ngx_uint_t direct_allowed);
static ngx_stream_trojan_mux_pending_frame_t *
ngx_stream_trojan_mux_alloc_pending_frame(
    ngx_stream_trojan_mux_stream_t *stream, size_t len);
static ngx_int_t ngx_stream_trojan_mux_queue_pending_frame(
    ngx_stream_trojan_mux_stream_t *stream, u_char *data, size_t len);
static ngx_int_t ngx_stream_trojan_mux_read_deferred_payload(
    ngx_stream_trojan_ctx_t *ctx, ngx_connection_t *c);
static ngx_int_t ngx_stream_trojan_mux_drain_pending_frames(
    ngx_stream_trojan_mux_stream_t *stream, ngx_uint_t *drained);
static size_t ngx_stream_trojan_mux_recycle_pending_frames(
    ngx_stream_trojan_mux_stream_t *stream);
static ngx_uint_t ngx_stream_trojan_mux_client_blocked(
    ngx_stream_trojan_ctx_t *ctx);
static ngx_uint_t ngx_stream_trojan_mux_client_blocked_ready(
    ngx_stream_trojan_ctx_t *ctx);
static void ngx_stream_trojan_mux_take_recv_tokens(
    ngx_stream_trojan_mux_stream_t *stream, size_t n);
static void ngx_stream_trojan_mux_return_recv_tokens(
    ngx_stream_trojan_mux_stream_t *stream, size_t n);
static void ngx_stream_trojan_mux_queue_update(
    ngx_stream_trojan_mux_stream_t *stream);
static size_t ngx_stream_trojan_mux_recycle_client_buffer(
    ngx_stream_trojan_mux_stream_t *stream);
...
static ngx_int_t ngx_stream_trojan_resolve_addr_prefer(ngx_pool_t *pool,
    ...
    ngx_addr_t *out);
static ngx_resolver_t *ngx_stream_trojan_session_resolver(
...
ngx_stream_trojan_handler(ngx_stream_session_t *s)
{
    ...
    ngx_queue_init(&ctx->mux_update_queue);
    ctx->mux_recv_tokens = NGX_STREAM_TROJAN_MUX_MAX_RECEIVE_BUFFER;
...
ngx_stream_trojan_websocket_send_parts(ngx_stream_trojan_ctx_t *ctx,
    u_char *src, size_t size, size_t headroom, u_char *next, size_t next_size,
    size_t frame_size, size_t *first_sent, size_t *second_sent)
{
    size_t             first_payload_sent, first_size, header_size;
    size_t             merged_sent, payload_sent, second_size;
    ngx_buf_t          hb, fb, sb;
    ...
    header_size = ctx->ws_send_header_len - ctx->ws_send_header_pos;
    if (first_size == 0 || header_size > headroom) {
        header_size = 0;
    }

    if (header_size) {
        ngx_memcpy(src - header_size,
                   ctx->ws_send_header + ctx->ws_send_header_pos,
                   header_size);
    }
    ...
    if (header_size == 0 && hb.pos < hb.last) {
        cl = &hcl;
        tail = &hcl;
    }
    ...
    if (header_size) {
        merged_sent = (size_t) (fb.pos - (src - header_size));
        if (merged_sent < header_size) {
            ctx->ws_send_header_pos += merged_sent;
            first_payload_sent = 0;
        } else {
            ctx->ws_send_header_pos += header_size;
            first_payload_sent = merged_sent - header_size;
        }
    } else {
        ctx->ws_send_header_pos = (size_t) (hb.pos - ctx->ws_send_header);
        first_payload_sent = (size_t) (fb.pos - src);
    }

    payload_sent = first_payload_sent;
    if (second_size) {
        payload_sent += (size_t) (sb.pos - next);
    }

    if (first_sent != NULL) {
        *first_sent = first_payload_sent;
    }
...
ngx_stream_trojan_client_send_part(ngx_stream_trojan_ctx_t *ctx, u_char *src,
    size_t size, size_t frame_size, size_t headroom)
{
    if (!ctx->websocket) {
    ...
    return ngx_stream_trojan_websocket_send_parts(
        ctx, src, size, headroom, NULL, 0, frame_size, NULL, NULL);
}
...
ngx_stream_trojan_client_send_parts(ngx_stream_trojan_ctx_t *ctx, u_char *src,
    size_t size, size_t headroom, const u_char *next, size_t next_size,
    size_t frame_size, size_t *first_sent, size_t *second_sent)
{
    size_t             payload_sent;
    ngx_buf_t          fb, sb;
    ...
    if (ctx->websocket) {
        return ngx_stream_trojan_websocket_send_parts(
            ctx, src, size, headroom, (u_char *) next, next_size, frame_size,
...
ngx_stream_trojan_client_send(ngx_stream_trojan_ctx_t *ctx, u_char *src,
    size_t size)
{
    return ngx_stream_trojan_client_send_part(ctx, src, size, size, 0);
}
    ...
    ngx_uint_t ip_prefer, uintptr_t udp_cache_key)
{
    ...
    data->type = type;
    data->ip_prefer = ip_prefer;
    data->udp_cache_key = udp_cache_key;
    data->port = addr->port;
    data->payload_len = payload_len;
...
ngx_stream_trojan_resolve_handler(ngx_resolver_ctx_t *rctx)
{
    ...
    if (data->type == ngx_stream_trojan_resolve_udp && rctx->valid > 0) {
        now = ngx_time();
        if (rctx->valid > now) {
            ttl = (ngx_msec_t) (rctx->valid - now) * 1000;
            if (ttl > NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_MAX_TTL) {
                ttl = NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_MAX_TTL;
            }
            data->udp_cache_expire = ngx_current_msec + ttl;
        }
    }
    ...
    ngx_resolver_addr_t *addrs, ngx_uint_t naddrs)
{
    ...
    if (ngx_stream_trojan_send_udp_resolved(ctx, data, addrs, naddrs)
        != NGX_OK)
    {
        ngx_stream_trojan_finalize(ctx, NGX_STREAM_BAD_GATEWAY);
        return;
...
static ngx_uint_t
ngx_stream_trojan_cache_hash_byte(ngx_uint_t hash, u_char ch)
{
    hash ^= ch;
    return hash * NGX_STREAM_TROJAN_CACHE_FNV_PRIME;
}

static ngx_uint_t ngx_stream_trojan_cache_seed;
static ngx_uint_t ngx_stream_trojan_cache_seeded;


static ngx_uint_t
ngx_stream_trojan_cache_set(ngx_uint_t hash, ngx_uint_t sets)
{
    if (!ngx_stream_trojan_cache_seeded) {
        ngx_stream_trojan_cache_seed = ngx_random();
        if (ngx_stream_trojan_cache_seed == 0) {
            ngx_stream_trojan_cache_seed =
                (ngx_uint_t) (uintptr_t) &ngx_stream_trojan_cache_seed;
        }
        ngx_stream_trojan_cache_seeded = 1;
    }

    hash ^= ngx_stream_trojan_cache_seed;
    hash ^= hash >> 16;
    hash *= 0x7feb352d;
    hash ^= hash >> 15;
    hash *= 0x846ca68b;
...
static ngx_uint_t
ngx_stream_trojan_route_target_hash(ngx_stream_trojan_addr_t *target)
{
    size_t      i;
    ngx_uint_t  hash;
    u_char      ch;

    hash = NGX_STREAM_TROJAN_CACHE_FNV_OFFSET;
    hash = ngx_stream_trojan_cache_hash_byte(hash, target->type);
    hash = ngx_stream_trojan_cache_hash_byte(hash,
                                             (u_char) target->host_len);
    hash = ngx_stream_trojan_cache_hash_byte(hash,
                                             (u_char) (target->host_len >> 8));
    hash = ngx_stream_trojan_cache_hash_byte(hash,
                                             (u_char) (target->port >> 8));
    hash = ngx_stream_trojan_cache_hash_byte(hash,
                                             (u_char) target->port);
    ...
    for (i = 0; i < target->host_len; i++) {
        ...
        if (target->type == NGX_STREAM_TROJAN_ADDR_DOMAIN) {
            ch = ngx_stream_trojan_route_lc(ch);
        }

        hash = ngx_stream_trojan_cache_hash_byte(hash, ch);
    }
    ...
    ngx_stream_trojan_addr_t *target, ngx_uint_t *found)
{
    ...
    *found = 0;
    ...
    hash = ngx_stream_trojan_route_target_hash(target);
    start = ngx_stream_trojan_cache_set(
                hash, NGX_STREAM_TROJAN_ROUTE_CACHE_SETS)
            * NGX_STREAM_TROJAN_ROUTE_CACHE_WAYS;
    e = conf->route_cache;

    for (i = 0; i < NGX_STREAM_TROJAN_ROUTE_CACHE_WAYS; i++) {
        if (!e[start + i].valid || e[start + i].hash != hash) {
            continue;
        }

        if (ngx_stream_trojan_route_target_equal(&e[start + i].target,
                                                 target))
        {
            e[start + i].accessed = ++conf->route_cache_generation;
            *found = 1;
            return e[start + i].route;
        }
    }
    ...
    ngx_stream_trojan_addr_t *target, ngx_stream_trojan_route_t *route)
{
    ...
    for (i = 0; i < NGX_STREAM_TROJAN_ROUTE_CACHE_WAYS; i++) {
        if (e[start + i].valid
            && e[start + i].hash == hash
            && ngx_stream_trojan_route_target_equal(&e[start + i].target,
                                                    target))
        {
            slot = &e[start + i];
            break;
...
static ngx_uint_t
ngx_stream_trojan_route_domain_hash(const u_char *value, size_t value_len)
{
    size_t      i;
    ngx_uint_t  hash;

    hash = NGX_STREAM_TROJAN_CACHE_FNV_OFFSET;
    hash = ngx_stream_trojan_cache_hash_byte(hash, (u_char) value_len);
    hash = ngx_stream_trojan_cache_hash_byte(hash, (u_char) (value_len >> 8));

    for (i = 0; i < value_len; i++) {
        hash = ngx_stream_trojan_cache_hash_byte(
            hash, ngx_stream_trojan_route_lc(value[i]));
    }
...
static ngx_uint_t
ngx_stream_trojan_route_domain_index_slots(ngx_uint_t count)
{
    ngx_uint_t  slots;
...
static ngx_int_t
ngx_stream_trojan_route_domain_index_insert(
    ngx_stream_trojan_route_domain_index_t *index, ngx_uint_t slots,
    ngx_str_t *value, ngx_uint_t route_index,
    ngx_stream_trojan_route_t *route)
{
    ngx_uint_t                              i, hash, idx;
    ngx_stream_trojan_route_domain_index_t *slot;

    hash = ngx_stream_trojan_route_domain_hash(value->data, value->len);
    idx = hash % slots;
...
static ngx_int_t
ngx_stream_trojan_route_build_domain_index(ngx_conf_t *cf,
    ngx_stream_trojan_srv_conf_t *conf)
{
    ngx_uint_t                       i, j, count, build_non_domain;
    ngx_stream_trojan_route_t       *route;
    ngx_stream_trojan_route_rule_t  *rule, **rule_slot;

    if (conf->route_domain_index != NULL || conf->routes == NULL) {
        return NGX_OK;
    }
    ...
    for (i = 0; i < conf->routes->nelts; i++) {
        ...
        for (j = 0; j < route[i].rules->nelts; j++) {
            if (rule[j].type == ngx_stream_trojan_route_rule_domain) {
                count++;
            }
        }
    }
    ...
    conf->route_domain_index_slots =
        ngx_stream_trojan_route_domain_index_slots(count);
    conf->route_domain_index = ngx_pcalloc(cf->pool,
        conf->route_domain_index_slots
        * sizeof(ngx_stream_trojan_route_domain_index_t));
    if (conf->route_domain_index == NULL) {
        return NGX_ERROR;
    }

    for (i = 0; i < conf->routes->nelts; i++) {
        rule = route[i].rules->elts;
        build_non_domain = route[i].non_domain_rules == NULL;
        ...
        for (j = 0; j < route[i].rules->nelts; j++) {
            if (rule[j].type != ngx_stream_trojan_route_rule_domain) {
                ...
                if (route[i].non_domain_rules == NULL) {
                    route[i].non_domain_rules = ngx_array_create(cf->pool, 1,
                ...
                rule_slot = ngx_array_push(route[i].non_domain_rules);
                if (rule_slot == NULL) {
                    return NGX_ERROR;
                }
            ...
            if (ngx_stream_trojan_route_domain_index_insert(
                    conf->route_domain_index, conf->route_domain_index_slots,
                    &rule[j].value, i, &route[i])
                != NGX_OK)
            {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                                   "could not index trojan_routes domains");
                return NGX_ERROR;
            }
        }
    }
    ...
    const u_char *value, size_t value_len, ngx_uint_t hash)
{
    ...
    if (conf->route_domain_index == NULL
        || conf->route_domain_index_slots == 0)
    {
        return NULL;
    }

    idx = hash % conf->route_domain_index_slots;

    for (i = 0; i < conf->route_domain_index_slots; i++) {
        slot = &conf->route_domain_index[(idx + i)
                                         % conf->route_domain_index_slots];
...
static ngx_stream_trojan_route_t *
ngx_stream_trojan_route_domain_index_match(ngx_stream_trojan_srv_conf_t *conf,
    ngx_stream_trojan_addr_t *target, ngx_uint_t *route_index)
{
    u_char                                   key[255];
    u_char                                  *suffix, *end, *p;
    size_t                                   host_len, suffix_len, i;
    ngx_uint_t                               hash, best;
    ngx_stream_trojan_route_t               *route;
    ngx_stream_trojan_route_domain_index_t  *slot;

    *route_index = (ngx_uint_t) -1;

    if (target->type != NGX_STREAM_TROJAN_ADDR_DOMAIN
        || target->host_len == 0 || target->host_len > sizeof(key)
        || conf->route_domain_index == NULL
        || conf->route_domain_index_slots == 0)
    {
        return NULL;
    }
    ...
    if (host_len > 1 && target->host[host_len - 1] == '.') {
        host_len--;
    }

    for (i = 0; i < host_len; i++) {
        key[i] = ngx_stream_trojan_route_lc(target->host[i]);
    }
    ...
    for (;;) {
        ...
        hash = ngx_stream_trojan_route_domain_hash(suffix, suffix_len);
        slot = ngx_stream_trojan_route_domain_index_find(conf, suffix,
                                                         suffix_len, hash);

        if (slot != NULL && slot->route_index < best) {
            best = slot->route_index;
            route = slot->route;
...
static ngx_uint_t
ngx_stream_trojan_udp_domain_cache_hash(const u_char *host, size_t host_len,
    uint16_t port, ngx_uint_t ip_prefer, uintptr_t resolver_id)
{
    size_t      i;
    ngx_uint_t  hash;
    ...
    hash = NGX_STREAM_TROJAN_CACHE_FNV_OFFSET;
    hash = ngx_stream_trojan_cache_hash_byte(hash,
                                             NGX_STREAM_TROJAN_ADDR_DOMAIN);
    hash = ngx_stream_trojan_cache_hash_byte(hash, (u_char) host_len);
    hash = ngx_stream_trojan_cache_hash_byte(hash, (u_char) (host_len >> 8));
    hash = ngx_stream_trojan_cache_hash_byte(hash, (u_char) (port >> 8));
    hash = ngx_stream_trojan_cache_hash_byte(hash, (u_char) port);
    hash = ngx_stream_trojan_cache_hash_byte(hash, (u_char) ip_prefer);
    hash = ngx_stream_trojan_cache_hash_byte(hash,
                                             (u_char) (ip_prefer >> 8));
    for (i = 0; i < sizeof(uintptr_t); i++) {
        hash = ngx_stream_trojan_cache_hash_byte(
            hash, (u_char) (resolver_id >> (i * 8)));
    }

    for (i = 0; i < host_len; i++) {
        hash = ngx_stream_trojan_cache_hash_byte(
            hash, ngx_stream_trojan_route_lc(host[i]));
    }
...
static ngx_int_t
ngx_stream_trojan_udp_domain_cache_host_equal(u_char *cached,
    const u_char *host, size_t host_len)
{
    size_t  i;

    for (i = 0; i < host_len; i++) {
        if (cached[i] != ngx_stream_trojan_route_lc(host[i])) {
            return 0;
        }
    }
...
static ngx_int_t
ngx_stream_trojan_udp_domain_cache_lookup(
    ngx_stream_trojan_srv_conf_t *conf, ngx_stream_trojan_addr_t *target,
    ngx_uint_t ip_prefer, uintptr_t resolver_id,
    struct sockaddr_storage *sockaddr, socklen_t *socklen)
{
    ngx_uint_t                                  i, start, hash;
    ngx_msec_t                                 now;
    ngx_stream_trojan_udp_domain_cache_entry_t *e;

    if (conf->udp_domain_cache == NULL
    ...
    hash = ngx_stream_trojan_udp_domain_cache_hash(
        target->host, target->host_len, target->port, ip_prefer, resolver_id);
    start = ngx_stream_trojan_cache_set(
                hash, NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_SETS)
            * NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_WAYS;
    now = ngx_current_msec;
    e = conf->udp_domain_cache;

    for (i = 0; i < NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_WAYS; i++) {
        if (!e[start + i].valid) {
            continue;
        }
        ...
        if (e[start + i].hash == hash
            && e[start + i].port == target->port
            && e[start + i].ip_prefer == ip_prefer
            && e[start + i].resolver_id == resolver_id
            && e[start + i].host_len == target->host_len
            && ngx_stream_trojan_udp_domain_cache_host_equal(
                   e[start + i].host, target->host, target->host_len))
        {
            if (e[start + i].socklen == 0
                || e[start + i].socklen
            ...
            ngx_memcpy(sockaddr, &e[start + i].sockaddr,
                       e[start + i].socklen);
            *socklen = e[start + i].socklen;
            e[start + i].accessed = ++conf->udp_domain_cache_generation;
            return NGX_OK;
        }
    }
...
static void
ngx_stream_trojan_udp_domain_cache_store(
    ngx_stream_trojan_srv_conf_t *conf, const u_char *host, size_t host_len,
    uint16_t port, ngx_uint_t ip_prefer, uintptr_t resolver_id,
    ngx_msec_t expire, struct sockaddr *sockaddr, socklen_t socklen)
{
    ngx_uint_t                                  i, start, oldest, hash;
    ngx_msec_t                                 now;
    ngx_stream_trojan_udp_domain_cache_entry_t *e, *slot;

    if (conf->udp_domain_cache == NULL
        || host == NULL || host_len == 0 || host_len > 255
        || expire == 0 || sockaddr == NULL || socklen == 0
        || socklen > (socklen_t) sizeof(struct sockaddr_storage))
    {
        return;
    }

    now = ngx_current_msec;
    if ((ngx_msec_int_t) (expire - now) <= 0) {
        return;
    }

    if ((ngx_msec_int_t) (expire - now)
        > NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_MAX_TTL)
    {
        expire = now + NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_MAX_TTL;
    }

    hash = ngx_stream_trojan_udp_domain_cache_hash(host, host_len, port,
                                                   ip_prefer, resolver_id);
    start = ngx_stream_trojan_cache_set(
                hash, NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_SETS)
            * NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_WAYS;
    e = conf->udp_domain_cache;
    ...
    for (i = 0; i < host_len; i++) {
        slot->host[i] = ngx_stream_trojan_route_lc(host[i]);
    }
    slot->port = port;
    slot->ip_prefer = ip_prefer;
    slot->resolver_id = resolver_id;
    ngx_memcpy(&slot->sockaddr, sockaddr, socklen);
    slot->socklen = socklen;
    slot->expire = expire;
    slot->accessed = ++conf->udp_domain_cache_generation;
    slot->valid = 1;
}
...
{
    ngx_uint_t                    i, start, cache_found, route_index;
    ngx_uint_t                    scan_limit, indexed;
    ngx_stream_trojan_route_t    *route, *cached_route, *candidate;
    ngx_stream_trojan_outbound_t *outbound;
    ...
    if (ctx->conf->route_enable) {
        ...
        indexed = target->type == NGX_STREAM_TROJAN_ADDR_DOMAIN
                  && ctx->conf->route_domain_index != NULL
                  && ctx->conf->route_domain_index_slots != 0
                  && target->host_len <= 255;
        start = 0;
        if (indexed) {
            for (start = 0; start < ctx->conf->routes->nelts; start++) {
                if (route[start].non_domain_rules == NULL
                    || route[start].non_domain_rules->nelts
                       != route[start].rules->nelts)
                {
                    break;
                }

                if (!ngx_stream_trojan_route_match(&route[start], target, 1)) {
                    continue;
                }

                ngx_stream_trojan_route_cache_store(ctx->conf, target,
                                                    &route[start]);
                outbound = ngx_stream_trojan_select_route_outbound(
                    &route[start], ctx->outbound, command, target);
                ngx_stream_trojan_route_cache_copy_target(
                    &ctx->route_cache_target, target);
                ctx->route_cache_command = command;
                ctx->route_cache_outbound = outbound;
        ...
        candidate = indexed
                    ? ngx_stream_trojan_route_domain_index_match(
                        ctx->conf, target, &route_index)
                    : NULL;
        scan_limit = candidate != NULL ? route_index : ctx->conf->routes->nelts;

        for (i = start; i < scan_limit; i++) {
            if (!ngx_stream_trojan_route_match(&route[i], target, indexed)) {
                continue;
            }
        ...
        if (candidate != NULL) {
            ngx_stream_trojan_route_cache_store(ctx->conf, target, candidate);
            outbound = ngx_stream_trojan_select_route_outbound(candidate,
                                                               ctx->outbound,
                                                               command, target);
            ngx_stream_trojan_route_cache_copy_target(
                &ctx->route_cache_target, target);
...
ngx_stream_trojan_route_match(ngx_stream_trojan_route_t *route,
    ngx_stream_trojan_addr_t *target, ngx_uint_t skip_domain)
{
    ngx_uint_t                       i;
    ngx_stream_trojan_route_rule_t  *rule, **rulep;
    ...
    if (skip_domain) {
        if (route->non_domain_rules == NULL
            || route->non_domain_rules->nelts == 0)
        {
            return 0;
        }

        rulep = route->non_domain_rules->elts;
        for (i = 0; i < route->non_domain_rules->nelts; i++) {
            if (ngx_stream_trojan_route_rule_match(rulep[i], target)) {
                return 1;
            }
        }
...
ngx_stream_trojan_init_proxy(ngx_stream_trojan_ctx_t *ctx)
{
    ...
    ctx->upstream_buffer = ngx_stream_trojan_create_temp_buf(
        c->pool, ctx->conf->buffer_size
                 + (ctx->websocket
                    ? NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE : 0));
    ...
    if (ctx->websocket) {
        ctx->upstream_buffer->start +=
            NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE;
        ctx->upstream_buffer->pos = ctx->upstream_buffer->start;
        ctx->upstream_buffer->last = ctx->upstream_buffer->start;
    }
    ...
    for ( ;; ) {
        while (buf->pos < buf->last) {
            if (dst == ctx->session->connection) {
                n = ngx_stream_trojan_client_send_part(
                    ctx, buf->pos, buf->last - buf->pos,
                    buf->last - buf->pos,
                    ctx->websocket && buf == ctx->upstream_buffer
                    ? NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE : 0);
            } else {
    ...
    ngx_connection_t *c)
{
    ...
    if (ctx->mux_payload_deferred) {
        return ngx_stream_trojan_mux_read_deferred_payload(ctx, c);
    }
    while (ctx->mux_payload_read < ctx->mux_payload_len) {
               ...
               != ngx_stream_trojan_mux_stream_closing)
        {
            ...
            if (ngx_stream_trojan_mux_ensure_client_buffer(stream, remaining)
                != NGX_OK)
            {
                return NGX_ERROR;
            }
        ...
        if (b != NULL) {
            b->last += n;
            ngx_stream_trojan_mux_take_recv_tokens(stream, (size_t) n);
    ...
    ngx_stream_trojan_mux_stream_t *stream, size_t len)
{
    ...
    if (ctx->mux_cool) {
        ...
        if (ngx_stream_trojan_mux_pack_header(
                ...
                sizeof(stream->frame_header)
                    - NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE,
                ctx->mux_version ? ctx->mux_version
                                 : NGX_STREAM_TROJAN_MUX_DEFAULT_VERSION,
    ...
    stream->frame_payload_len = len;
    stream->frame_payload_sent = 0;
    return NGX_OK;
}
    ...
    size_t limit)
{
    ...
    size_t                    payload_delta, payload_remaining;
    ...
    while (stream->frame_header_pos < stream->frame_header_len
           || stream->frame_payload_sent < stream->frame_payload_len)
    {
        if (ctx->websocket) {
            ...
            payload_remaining = stream->frame_payload_len
                                - stream->frame_payload_sent;
            available = payload_remaining;
            ...
            n = ngx_stream_trojan_websocket_send_parts(
                ctx,
                stream->frame_header
                    + NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE
                    + stream->frame_header_pos,
                header_available, NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE,
                b->pos, available, frame_size, &header_sent, &payload_sent);
            ...
            b->pos += payload_sent;
            stream->frame_payload_sent += payload_sent;
            stream->tx_written += (uint32_t) payload_sent;
            if (sent != NULL) {
        ...
        available = stream->frame_payload_len - stream->frame_payload_sent;
        if (limit && sent != NULL) {
        ...
        hb.pos = stream->frame_header
                 + NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE
                 + stream->frame_header_pos;
        hb.last = stream->frame_header
                  + NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE
                  + stream->frame_header_len;
        hb.memory = 1;
        hcl.buf = &hb;
        ...
        stream->frame_header_pos = (size_t) (
            hb.pos
            - (stream->frame_header
               + NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE));
        b->pos = pb.pos;
        payload_delta = (size_t) (b->pos - payload_before);
        stream->frame_payload_sent += payload_delta;
        stream->tx_written += (uint32_t) payload_delta;
...
static ngx_int_t
ngx_stream_trojan_mux_stream_send_window(
    ngx_stream_trojan_mux_stream_t *stream, size_t *window)
{
    int32_t                   inflight;
    uint32_t                  available;
    ngx_stream_trojan_ctx_t  *ctx;
    ...
    ctx = stream->ctx;
    if (ctx->mux_cool || ctx->mux_version != NGX_STREAM_TROJAN_MUX_VERSION2) {
        *window = NGX_STREAM_TROJAN_MUX_MAX_FRAME_SIZE;
        return NGX_OK;
    }

    inflight = (int32_t) (stream->tx_written - stream->tx_peer_consumed);
    if (inflight < 0) {
        return NGX_ERROR;
    }

    if (stream->tx_peer_window <= (uint32_t) inflight) {
        *window = 0;
        return NGX_AGAIN;
    }

    available = stream->tx_peer_window - (uint32_t) inflight;
    if (available > NGX_STREAM_TROJAN_MUX_MAX_FRAME_SIZE) {
        available = NGX_STREAM_TROJAN_MUX_MAX_FRAME_SIZE;
    }
    ...
    ngx_stream_trojan_mux_stream_t *stream, size_t *sent, size_t limit)
{
    ...
    while (b && b->pos < b->last) {
        if (stream->frame_header_len == 0) {
            rc = ngx_stream_trojan_mux_stream_send_window(stream, &window);
            if (rc == NGX_ERROR) {
                return NGX_ERROR;
            }
            if (rc == NGX_AGAIN || window == 0) {
                return NGX_BUSY;
            }
            ...
            if (len > NGX_STREAM_TROJAN_MUX_MAX_FRAME_SIZE) {
                len = NGX_STREAM_TROJAN_MUX_MAX_FRAME_SIZE;
            }
...
ngx_stream_trojan_process_mux(ngx_stream_trojan_ctx_t *ctx)
{
    ...
    if (ngx_stream_trojan_update_read_event(c,
            ngx_stream_trojan_mux_client_blocked(ctx))
        != NGX_OK
        || ngx_handle_write_event(c->write, 0) != NGX_OK)
    {
        ngx_stream_trojan_finalize(ctx, NGX_STREAM_INTERNAL_SERVER_ERROR);
...
ngx_stream_trojan_mux_read_client(ngx_stream_trojan_ctx_t *ctx)
{
    ...
    for ( ;; ) {
        ...
        if (!ctx->mux_frame_parsed) {
            ...
            if (ctx->mux_version == 0) {
                ctx->mux_version = ctx->mux_frame.version;

            } else if (ctx->mux_frame.version != ctx->mux_version) {
                return NGX_ERROR;
            }
            ctx->mux_payload_len = ctx->mux_frame.length;
            ctx->mux_payload_read = 0;
            ctx->mux_payload_feed_pos = 0;
            ctx->mux_payload_stream = NULL;
            ctx->mux_payload_direct = 0;
            ctx->mux_payload_accept_checked = 0;
            ctx->mux_payload_blocked = 0;
            ctx->mux_payload_deferred = 0;
            ...
            if (ctx->mux_frame.command == NGX_STREAM_TROJAN_MUX_CMD_PSH) {
                ...
                if (stream != NULL) {
                    rc = ngx_stream_trojan_mux_prepare_client_payload(
                        ctx, stream, ctx->mux_payload_len,
        ...
        {
            rc = ngx_stream_trojan_mux_prepare_client_payload(
                ctx, ctx->mux_payload_stream, ctx->mux_payload_len, 1);
            if (rc != NGX_OK) {
                return rc;
            }
        }
        ...
        if (ctx->mux_payload_read < ctx->mux_payload_len) {
            return NGX_OK;
        }
...
static ngx_int_t
ngx_stream_trojan_mux_pack_update_frame(
    ngx_stream_trojan_mux_stream_t *stream)
{
    if (ngx_stream_trojan_mux_pack_header(stream->update_frame,
            sizeof(stream->update_frame), NGX_STREAM_TROJAN_MUX_VERSION2,
            NGX_STREAM_TROJAN_MUX_CMD_UPD, NGX_STREAM_TROJAN_MUX_UPD_LEN,
            stream->id)
        != NGX_STREAM_TROJAN_MUX_OK)
    {
        return NGX_ERROR;
    ...
    if (ngx_stream_trojan_mux_pack_update(
            stream->update_frame + NGX_STREAM_TROJAN_MUX_HEADER_LEN,
            NGX_STREAM_TROJAN_MUX_UPD_LEN, stream->update_consumed,
            NGX_STREAM_TROJAN_MUX_MAX_STREAM_BUFFER)
        != NGX_STREAM_TROJAN_MUX_OK)
    {
        return NGX_ERROR;
    }

    stream->update_frame_len = NGX_STREAM_TROJAN_MUX_HEADER_LEN
                               + NGX_STREAM_TROJAN_MUX_UPD_LEN;
    stream->update_frame_pos = 0;
...
static ngx_int_t
ngx_stream_trojan_mux_flush_updates(ngx_stream_trojan_ctx_t *ctx,
    size_t *sent, size_t limit)
{
    ngx_int_t                       rc;
    ngx_queue_t                    *q;
    ngx_connection_t               *c;
    ...
    while (!ngx_queue_empty(&ctx->mux_update_queue)) {
        q = ngx_queue_head(&ctx->mux_update_queue);
        ngx_queue_remove(q);
        stream = ngx_queue_data(q, ngx_stream_trojan_mux_stream_t,
                                update_queue);
        stream->update_queued = 0;

        if (stream->update_frame_len == 0) {
            if (ngx_stream_trojan_mux_pack_update_frame(stream) != NGX_OK) {
                return NGX_ERROR;
            }
        }

        rc = ngx_stream_trojan_mux_send_bytes(ctx, c, stream->update_frame,
                                              &stream->update_frame_pos,
                                              stream->update_frame_len, sent);
        if (rc == NGX_ERROR) {
            return NGX_ERROR;
        }

        if (rc == NGX_AGAIN) {
            ngx_queue_insert_tail(&ctx->mux_update_queue,
                                  &stream->update_queue);
            stream->update_queued = 1;
            return NGX_AGAIN;
        }

        stream->update_frame_len = 0;
        stream->update_frame_pos = 0;

        if (stream->update_resend) {
            stream->update_resend = 0;
            ngx_stream_trojan_mux_queue_update(stream);
        }
...
ngx_stream_trojan_mux_flush_client(ngx_stream_trojan_ctx_t *ctx)
{
    ...
    rc = ngx_stream_trojan_mux_flush_updates(ctx, &sent, limit);
    if (rc == NGX_ERROR) {
        return NGX_ERROR;
    }
    if (rc == NGX_AGAIN) {
...
ngx_stream_trojan_mux_handle_frame(ngx_stream_trojan_ctx_t *ctx)
{
    ...
    case NGX_STREAM_TROJAN_MUX_CMD_PSH:
        ...
        if (stream != NULL) {
            ...
            if (ctx->mux_payload_feed_pos > ctx->mux_payload_len) {
                return NGX_ERROR;
            }

            rc = ngx_stream_trojan_mux_feed_stream(
                stream, ctx->mux_payload + ctx->mux_payload_feed_pos,
                ctx->mux_payload_len - ctx->mux_payload_feed_pos);
            if (rc == NGX_AGAIN) {
                ctx->mux_payload_blocked = 1;
            }
            return rc;
    ...
    case NGX_STREAM_TROJAN_MUX_CMD_UPD:
        if (ngx_stream_trojan_mux_parse_update(ctx->mux_payload,
                ctx->mux_payload_len, &consumed, &window)
            != NGX_STREAM_TROJAN_MUX_OK)
        {
            return NGX_ERROR;
        }
        ...
        if ((int32_t) (consumed - stream->tx_written) > 0) {
            return NGX_ERROR;
        }

        stream->tx_peer_consumed = consumed;
        stream->tx_peer_window = window;

        if ((stream->upstream_buffer
             && stream->upstream_buffer->pos < stream->upstream_buffer->last)
            || stream->fin_to_client || stream->close_after_send)
        {
            ngx_stream_trojan_mux_queue_flush(stream);
        }
...
ngx_stream_trojan_mux_cool_handle_frame(ngx_stream_trojan_ctx_t *ctx)
{
    ...
    case NGX_STREAM_TROJAN_MUX_COOL_STATUS_NEW:
        ...
        if (ctx->mux_payload_len
            && ctx->mux_recv_tokens < ctx->mux_payload_len)
        {
            ctx->mux_payload_blocked = 1;
            return NGX_AGAIN;
        }
        ...
        if (ctx->mux_payload_len) {
            ...
            rc = ngx_stream_trojan_mux_append_client_data(
...
ngx_stream_trojan_mux_create_stream(ngx_stream_trojan_ctx_t *ctx, uint32_t id)
{
    ...
    ngx_queue_init(&stream->update_queue);
    ngx_queue_init(&stream->pending_frames);
    stream->tx_peer_window = NGX_STREAM_TROJAN_MUX_INITIAL_PEER_WINDOW;
    ...
    ngx_stream_trojan_mux_stream_t *stream, size_t len)
{
    ...
    if (stream->ctx->mux_recv_tokens < len) {
        return NGX_AGAIN;
    }
...
static ngx_int_t
ngx_stream_trojan_mux_prepare_client_payload(ngx_stream_trojan_ctx_t *ctx,
    ngx_stream_trojan_mux_stream_t *stream, size_t len,
    ngx_uint_t direct_allowed)
{
    ngx_int_t  rc;
    ...
    if (!ngx_queue_empty(&stream->pending_frames)) {
        if (ctx->mux_recv_tokens < len) {
            ctx->mux_payload_blocked = 1;
            return NGX_AGAIN;
        }

        ctx->mux_payload_direct = 0;
        ctx->mux_payload_deferred = 1;
    ...
    ngx_stream_trojan_mux_stream_t *stream, size_t len)
{
    ...
    ngx_stream_trojan_mux_pending_frame_t  *frame;

    capacity = stream->ctx->conf->buffer_size;
    if (capacity > NGX_STREAM_TROJAN_MUX_MAX_FRAME_SIZE) {
        capacity = NGX_STREAM_TROJAN_MUX_MAX_FRAME_SIZE;
    }
    if (capacity < len) {
        capacity = len;
    }

    size = sizeof(ngx_stream_trojan_mux_pending_frame_t) + capacity - 1;
    frame = ngx_alloc(size, stream->ctx->session->connection->log);
    if (frame == NULL) {
        return NULL;
    }
...
static ngx_int_t
ngx_stream_trojan_mux_queue_pending_frame(
    ngx_stream_trojan_mux_stream_t *stream, u_char *data, size_t len)
{
    size_t                                  available, n;
    ngx_queue_t                            *q;
    ...
    while (len) {
        ...
        if (!ngx_queue_empty(&stream->pending_frames)) {
            q = stream->pending_frames.prev;
            frame = ngx_queue_data(q, ngx_stream_trojan_mux_pending_frame_t,
                                   queue);
        ...
        if (frame == NULL) {
            frame = ngx_stream_trojan_mux_alloc_pending_frame(stream, len);
            if (frame == NULL) {
                return NGX_ERROR;
            }

            ngx_queue_insert_tail(&stream->pending_frames, &frame->queue);
        }

        available = frame->capacity - frame->len;
        n = available < len ? available : len;
        ngx_memcpy(frame->data + frame->len, data, n);
        frame->len += n;
        data += n;
        len -= n;

        ngx_stream_trojan_mux_take_recv_tokens(stream, n);
    }
...
static ngx_int_t
ngx_stream_trojan_mux_read_deferred_payload(ngx_stream_trojan_ctx_t *ctx,
    ngx_connection_t *c)
{
    ssize_t                                n;
    size_t                                 available, remaining;
    ...
    stream = ctx->mux_payload_stream;
    if (stream == NULL || ctx->mux_payload_len == 0
        || stream->state == ngx_stream_trojan_mux_stream_closing)
    {
        ctx->mux_payload_deferred = 0;
        return ngx_stream_trojan_mux_read_client_payload(ctx, c);
    }

    while (ctx->mux_payload_read < ctx->mux_payload_len) {
        if (ctx->mux_recv_tokens == 0) {
            ctx->mux_payload_blocked = 1;
            return NGX_AGAIN;
        }
        ...
        if (frame == NULL) {
            remaining = ctx->mux_payload_len - ctx->mux_payload_read;
            frame = ngx_stream_trojan_mux_alloc_pending_frame(stream,
                                                              remaining);
            if (frame == NULL) {
                return NGX_ERROR;
            }
        ...
        if (available > ctx->mux_recv_tokens) {
            available = ctx->mux_recv_tokens;
        }
        if (available == 0) {
            ctx->mux_payload_blocked = 1;
            return NGX_AGAIN;
        ...
        ctx->mux_payload_blocked = 0;
        n = ngx_stream_trojan_client_recv(ctx, frame->data + frame->len,
                                          available);
...
static ngx_int_t
ngx_stream_trojan_mux_drain_pending_frames(
    ngx_stream_trojan_mux_stream_t *stream, ngx_uint_t *drained)
{
    size_t                                  available, remaining;
    ngx_buf_t                              *b;
    ...
    *drained = 0;
    ...
    while (!ngx_queue_empty(&stream->pending_frames)) {
        q = ngx_queue_head(&stream->pending_frames);
        frame = ngx_queue_data(q, ngx_stream_trojan_mux_pending_frame_t,
                               queue);

        remaining = frame->len - frame->pos;
        if (remaining == 0) {
            ngx_queue_remove(q);
            ngx_free(frame);
            continue;
        ...
        ngx_memcpy(b->last, frame->data + frame->pos, available);
        b->last += available;
        frame->pos += available;
        *drained = 1;
...
static size_t
ngx_stream_trojan_mux_recycle_pending_frames(
    ngx_stream_trojan_mux_stream_t *stream)
{
    size_t                                  n;
    ngx_queue_t                            *q;
    ...
    while (!ngx_queue_empty(&stream->pending_frames)) {
        ...
        if (frame->len > frame->pos) {
            n += frame->len - frame->pos;
        }
        ngx_free(frame);
    }
    ...
    ctx = stream->ctx;
    if (ctx->mux_recv_tokens > NGX_STREAM_TROJAN_MUX_MAX_RECEIVE_BUFFER - n) {
        ctx->mux_recv_tokens = NGX_STREAM_TROJAN_MUX_MAX_RECEIVE_BUFFER;
    } else {
        ctx->mux_recv_tokens += n;
    }

    if (ngx_stream_trojan_mux_client_blocked_ready(ctx)) {
...
static void
ngx_stream_trojan_mux_take_recv_tokens(ngx_stream_trojan_mux_stream_t *stream,
    size_t n)
{
    ngx_stream_trojan_ctx_t  *ctx;
    ...
    ctx = stream->ctx;
    if (ctx->mux_recv_tokens > n) {
        ctx->mux_recv_tokens -= n;
    } else {
        ctx->mux_recv_tokens = 0;
    }
}


static void
ngx_stream_trojan_mux_return_recv_tokens(ngx_stream_trojan_mux_stream_t *stream,
    size_t n)
{
    uint32_t                  old;
    ngx_stream_trojan_ctx_t  *ctx;
    ...
    if (ctx->mux_version != NGX_STREAM_TROJAN_MUX_VERSION2) {
        return;
    }

    old = stream->rx_consumed;
    stream->rx_consumed += (uint32_t) n;
    stream->rx_since_update += (uint32_t) n;

    if (old == 0 || stream->update_consumed == 0
        || stream->rx_since_update
           >= NGX_STREAM_TROJAN_MUX_WINDOW_UPDATE_THRESHOLD)
    {
        stream->update_consumed = stream->rx_consumed;
        stream->rx_since_update = 0;
        ngx_stream_trojan_mux_queue_update(stream);
    }
}
...
static void
ngx_stream_trojan_mux_queue_update(ngx_stream_trojan_mux_stream_t *stream)
{
    ngx_stream_trojan_ctx_t  *ctx;
    ...
    if (!stream->update_queued) {
        ngx_queue_insert_tail(&ctx->mux_update_queue, &stream->update_queue);
        stream->update_queued = 1;
        return;
    }

    if (stream->update_frame_pos == 0) {
        stream->update_frame_len = 0;
        return;
    }

    stream->update_resend = 1;
}


static size_t
ngx_stream_trojan_mux_recycle_client_buffer(
    ngx_stream_trojan_mux_stream_t *stream)
{
    size_t                    n;
    ngx_buf_t                *b;
    ...
    if (stream == NULL || stream->client_buffer == NULL) {
        return 0;
    }
...
static ngx_uint_t
ngx_stream_trojan_mux_client_blocked(ngx_stream_trojan_ctx_t *ctx)
{
    return ctx != NULL
...
static ngx_uint_t
ngx_stream_trojan_mux_client_blocked_ready(ngx_stream_trojan_ctx_t *ctx)
{
    size_t  needed;

    if (!ngx_stream_trojan_mux_client_blocked(ctx)) {
        return 0;
    }

    if (ctx->mux_payload_deferred) {
        return ctx->mux_recv_tokens != 0;
    }

    if (ctx->mux_payload_feed_pos < ctx->mux_payload_len) {
        needed = ctx->mux_payload_len - ctx->mux_payload_feed_pos;
    } else {
        needed = ctx->mux_payload_len;
    }

    return ctx->mux_recv_tokens >= needed;
}
    ...
    ngx_stream_trojan_mux_stream_t *stream, u_char *data, size_t len)
{
    ...
    if (!ngx_queue_empty(&stream->pending_frames)) {
        return ngx_stream_trojan_mux_queue_pending_frame(stream, data, len);
    }

    rc = ngx_stream_trojan_mux_stream_can_accept(stream, len);
    if (rc == NGX_AGAIN && stream->ctx->mux_recv_tokens >= len) {
        return ngx_stream_trojan_mux_queue_pending_frame(stream, data, len);
    }
    if (rc != NGX_OK) {
        return rc;
    ...
    b->last += len;
    ngx_stream_trojan_mux_take_recv_tokens(stream, len);
    ...
    while (len) {
        ...
        if (stream->state == ngx_stream_trojan_mux_stream_request) {
            for ( ;; ) {
                ...
                data += n;
                len -= n;
                stream->ctx->mux_payload_feed_pos += n;
    ...
    ngx_stream_trojan_mux_stream_t *stream, size_t *send_budget)
{
    ...
    while (b->pos < b->last) {
        ...
        b->pos += n;
        ngx_stream_trojan_mux_return_recv_tokens(stream, (size_t) n);
        refreshed = 1;
        if (send_budget != NULL) {
    ...
    if (stream->client_fin && !stream->upstream_write_shutdown
        && ngx_queue_empty(&stream->pending_frames))
    {
        (void) shutdown(pc->fd, SHUT_WR);
    ...
    size_t *send_budget)
{
    ...
    for ( ;; ) {
        ...
        if (ngx_stream_trojan_mux_drain_pending_frames(stream, &drained)
            != NGX_OK)
        {
            ngx_stream_trojan_mux_close_stream(stream, 1);
            return NGX_ERROR;
    ...
    if (ngx_stream_trojan_mux_client_blocked_ready(stream->ctx)) {
...
ngx_stream_trojan_mux_cleanup_stream(ngx_stream_trojan_mux_stream_t *stream)
{
    ...
    ctx = stream->ctx;
    pool = stream->pool;
    (void) ngx_stream_trojan_mux_recycle_client_buffer(stream);
    (void) ngx_stream_trojan_mux_recycle_pending_frames(stream);
    ...
    if (stream->update_queued) {
        ngx_queue_remove(&stream->update_queue);
        stream->update_queued = 0;
    }
...
ngx_stream_trojan_session_resolver(ngx_stream_session_t *s)
{
    ngx_stream_core_srv_conf_t *cscf;

    cscf = ngx_stream_get_module_srv_conf(s, ngx_stream_core_module);

    if (cscf->resolver == NULL || cscf->resolver->connections.nelts == 0) {
        return NULL;
    }
...
{
    return ngx_stream_trojan_session_resolver(s) != NULL;
}
    ...
    ngx_stream_trojan_udp_frame_t *frame)
{
    ...
    case NGX_STREAM_TROJAN_ADDR_DOMAIN:
        ...
        if (dns_rule != NULL) {
            if (ngx_stream_trojan_udp_domain_cache_lookup(
                    ctx->conf, &frame->addr, dns_rule->ip_prefer,
                    (uintptr_t) dns_rule, &ss, &socklen)
                == NGX_OK)
            {
                return ngx_stream_trojan_send_udp_sockaddr(
                    ctx, (struct sockaddr *) &ss, socklen,
        ...
        ip_prefer = ngx_stream_trojan_current_ip_prefer(ctx);
            ...
            && frame->addr.host_len > 0 && frame->addr.host_len <= 255)
        {
            ...
            if (ngx_stream_trojan_udp_domain_cache_lookup(
                    ctx->conf, &frame->addr, ip_prefer,
                    (uintptr_t) ctx->conf->doh_conf, &ss, &socklen)
                == NGX_OK)
            {
                return ngx_stream_trojan_send_udp_sockaddr(
                    ctx, (struct sockaddr *) &ss, socklen,
            ...
            udata = ngx_stream_trojan_create_doh_resolve_data(ctx,
                        ...
                        ip_prefer, ctx->conf->doh_conf);
            if (udata == NULL) {
                return NGX_ERROR;
            }
            udata->udp_cache_key = (uintptr_t) ctx->conf->doh_conf;
        ...
        resolver = ngx_stream_trojan_session_resolver(ctx->session);
        if (ngx_stream_trojan_use_nginx_resolver(frame->addr.type,
                                                 resolver != NULL))
        {
            if (ngx_stream_trojan_udp_domain_cache_lookup(
                    ctx->conf, &frame->addr, ip_prefer,
                    (uintptr_t) resolver, &ss, &socklen)
                == NGX_OK)
            {
                return ngx_stream_trojan_send_udp_sockaddr(
                    ctx, (struct sockaddr *) &ss, socklen,
...
ngx_stream_trojan_send_udp_resolved(ngx_stream_trojan_ctx_t *ctx,
    ngx_stream_trojan_resolve_data_t *data, ngx_resolver_addr_t *addrs,
    ngx_uint_t naddrs)
{
    ...
    resolved = ngx_stream_trojan_resolver_pick_addr(addrs, naddrs,
                                                    data->ip_prefer);
    if (resolved == NULL || resolved->socklen == 0
        || resolved->socklen > sizeof(ss))
    {
        return NGX_ERROR;
    }

    ngx_memcpy(&ss, resolved->sockaddr, resolved->socklen);
    ngx_inet_set_port((struct sockaddr *) &ss, data->port);

    rc = ngx_stream_trojan_send_udp_sockaddr(ctx, (struct sockaddr *) &ss,
                                             resolved->socklen, data->payload,
                                             data->payload_len);
    if (rc == NGX_OK) {
        ngx_stream_trojan_udp_domain_cache_store(
            ctx->conf, data->name, data->name_len, data->port,
            data->ip_prefer, data->udp_cache_key, data->udp_cache_expire,
            (struct sockaddr *) &ss, resolved->socklen);
    }
    ...
    size_t header_len, const u_char *payload, size_t payload_len)
{
    ...
    if (ctx->udp_pending_to_client == NULL) {
        ...
        ctx->udp_pending_to_client->start +=
            NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE;
        ctx->udp_pending_to_client->pos =
            ctx->udp_pending_to_client->start;
        ctx->udp_pending_to_client->last =
            ctx->udp_pending_to_client->start;
    }
...
ngx_stream_trojan_flush_udp_client(ngx_stream_trojan_ctx_t *ctx)
{
           ...
           || ctx->udp_pending_payload_sent < ctx->udp_pending_payload_len)
    {
        ...
        n = ngx_stream_trojan_client_send_parts(
            ctx, b->pos, b->last - b->pos,
            NGX_STREAM_TROJAN_WS_SEND_HEADER_SIZE, payload,
...
ngx_stream_trojan_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->routes == prev->routes && conf->route_domain_index == NULL) {
        conf->route_domain_index = prev->route_domain_index;
        conf->route_domain_index_slots = prev->route_domain_index_slots;
    }
    ...
    if ((conf->enable || (conf->socks5_enable && conf->socks5_udp_enable))
        && conf->udp_domain_cache == NULL)
    {
        conf->udp_domain_cache = ngx_pcalloc(cf->pool,
            NGX_STREAM_TROJAN_UDP_DOMAIN_CACHE_ENTRIES
            * sizeof(ngx_stream_trojan_udp_domain_cache_entry_t));
        if (conf->udp_domain_cache == NULL) {
            return NGX_CONF_ERROR;
        }
    }
    ...
    ngx_stream_trojan_srv_conf_t *conf)
{
    ...
    if (ngx_stream_trojan_route_build_domain_index(cf, conf) != NGX_OK) {
        return NGX_ERROR;
    }
