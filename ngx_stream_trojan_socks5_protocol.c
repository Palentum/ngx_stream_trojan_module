...
static int
ngx_stream_trojan_socks5_addr_len(const uint8_t *buf, size_t len,
    size_t *addr_len)
{
    *addr_len = 0;
...
int
ngx_stream_trojan_socks5_build_greeting(uint8_t *out, size_t out_len,
    int userpass, size_t *written)
{
    ...
    if (userpass) {
        ...
        out[1] = 2;
        out[2] = NGX_STREAM_TROJAN_SOCKS5_METHOD_NO_AUTH;
        out[3] = NGX_STREAM_TROJAN_SOCKS5_METHOD_USERPASS;
        *written = 4;
        return 0;
    }
...
int
ngx_stream_trojan_socks5_parse_method_response(const uint8_t *buf,
    size_t len, int userpass)
{
    if (buf == NULL || len < 2 || buf[0] != NGX_STREAM_TROJAN_SOCKS5_VERSION) {
        return -1;
    }

    if (buf[1] == NGX_STREAM_TROJAN_SOCKS5_METHOD_NO_AUTH) {
        return 0;
    }

    if (userpass && buf[1] == NGX_STREAM_TROJAN_SOCKS5_METHOD_USERPASS) {
        return 1;
    }
...
int
ngx_stream_trojan_socks5_build_auth(const uint8_t *username,
    ...
    uint8_t *out, size_t out_len, size_t *written)
{
    ...
    out[0] = NGX_STREAM_TROJAN_SOCKS5_AUTH_VERSION;
...
int
ngx_stream_trojan_socks5_parse_auth_response(const uint8_t *buf, size_t len)
{
    if (buf == NULL || len < 2 || buf[0] != NGX_STREAM_TROJAN_SOCKS5_AUTH_VERSION) {
        return -1;
    }
...
int
ngx_stream_trojan_socks5_response_len(const uint8_t *buf, size_t len,
    size_t *needed)
{
    size_t addr_len;
    int rc;
...
int
ngx_stream_trojan_socks5_parse_response(const uint8_t *buf, size_t len,
    ngx_stream_trojan_addr_t *addr)
{
    size_t needed;

    if (addr == NULL
        || ngx_stream_trojan_socks5_response_len(buf, len, &needed) != 0
        || len < needed || buf[1] != NGX_STREAM_TROJAN_SOCKS5_STATUS_OK)
    {
        return -1;
    }
...
int
ngx_stream_trojan_socks5_parse_udp_packet(const uint8_t *buf, size_t len,
