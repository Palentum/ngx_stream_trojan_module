#ifndef _NGX_STREAM_TROJAN_SOCKS5_PROTOCOL_H_INCLUDED_
#define _NGX_STREAM_TROJAN_SOCKS5_PROTOCOL_H_INCLUDED_
...
#define NGX_STREAM_TROJAN_SOCKS5_VERSION 0x05
#define NGX_STREAM_TROJAN_SOCKS5_AUTH_VERSION 0x01
#define NGX_STREAM_TROJAN_SOCKS5_METHOD_NO_AUTH 0x00
#define NGX_STREAM_TROJAN_SOCKS5_METHOD_USERPASS 0x02
#define NGX_STREAM_TROJAN_SOCKS5_METHOD_NONE 0xff
#define NGX_STREAM_TROJAN_SOCKS5_CMD_CONNECT 0x01
#define NGX_STREAM_TROJAN_SOCKS5_CMD_UDP_ASSOCIATE 0x03
#define NGX_STREAM_TROJAN_SOCKS5_STATUS_OK 0x00
#define NGX_STREAM_TROJAN_SOCKS5_STATUS_GENERAL_FAILURE 0x01
#define NGX_STREAM_TROJAN_SOCKS5_MAX_PACKET \
    (3 + NGX_STREAM_TROJAN_MAX_ADDR_LEN + NGX_STREAM_TROJAN_MAX_UDP_PAYLOAD)

int ngx_stream_trojan_socks5_build_greeting(uint8_t *out, size_t out_len,
    int userpass, size_t *written);
int ngx_stream_trojan_socks5_parse_method_response(const uint8_t *buf,
    size_t len, int userpass);
int ngx_stream_trojan_socks5_build_auth(const uint8_t *username,
...
int ngx_stream_trojan_socks5_parse_udp_packet(const uint8_t *buf, size_t len,
